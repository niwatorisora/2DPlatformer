...
Unity 6000.4.5f1 で作られた 2D プラットフォーマーのプロトタイプです。プレイヤー移動・マウス照準射撃・マガジン/リロード・弾プール・ScriptableObject 駆動の弾/武器データ・陣営ベースの味方弾フィルタリング・敵撃破スコア・State/Factory パターンによる敵 AI が実装されています。

主なシーン: `Assets/Scenes/MainScene1.unity`（ゲームプレイ検証用）、`Assets/Scenes/SampleScene.unity`（最小プロトタイプ）
...
- **プレイヤー**: 左右移動（`PlayerMovement`）、マウス照準射撃（`PlayerShooter`）、`R` キー手動リロード
- **弾数管理**: `Magazine`（マガジン残弾 / 予備弾 / リロード）、設定は `WeaponData`、状態はランタイム component
...
- **スコア**: `EnemyData.scoreValue` → `EnemyController.OnEnemyKilled` → `ScoreManager`
- **UI**: `AmmoHudView`（残弾 HUD + 武器アイコン）、`ScoreHudView`（スコア HUD）、`HpBarUI`（ワールド空間 HP バー）
- **キャラクタービジュアル**: `CharacterVisualController`（`VisualRoot` 子の回転固定・水平フリップ）、スプライトは Prefab の `VisualRoot/SpriteRenderer`、武器スプライトは `WeaponData.weaponSprite`
...
- 敵への弾数制限 → 敵 Prefab に `Magazine` を追加し `EnemyShooterAttack` から消費する
- スコア倍率・コンボ → `ScoreManager.AddScore` の前段に乗算ロジックを追加
