...
public class WaveSpawner : MonoBehaviour
{
    ...
    void Spawn(WaveTable.SpawnRow row, int waveIndex, int rowIndex)
    {
        ...
        EnemySkin.Selection skinSelection = skin != null
            ? skin.PickRandom()
            : new EnemySkin.Selection(null, 1f);
        MovementProfile movementProfile = row.movementProfile ?? row.enemy.defaultMovementProfile;
        AttackProfile attackProfile = row.attackProfile ?? row.enemy.defaultAttackProfile;
        if (!enemy.Initialize(row.enemy, playerTarget, bulletPool, cachedKillListeners,
                HandleEnemyDespawned, skinSelection.Sprite, skinSelection.Scale, movementProfile, attackProfile))
        {
